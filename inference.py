...
def load_model(weight_path, num_classes):
    ...
    model.load_state_dict(weight, strict=True)  # strict=True로 변경
...
def Convert_ONNX():
    model = load_model(config["WEIGHT"], len(class_names))[0]
    # set the model to inference mode
    model = model.cpu()
    model.eval()
    # Let's create a dummy input tensor
    dummy_input = torch.randn(1, 3, 384, 384)  # shape : (3,384,384)

    # Export the model
    torch.onnx.export(
        model,  # model being run
        dummy_input,  # model input (or a tuple for multiple inputs)
        "Bestefficientnet.onnx",  # where to save the model
...
def predict_onnx(image_path, onnx_model_path):
    """ONNX 모델로 이미지 예측"""
    # ONNX 세션 생성
    ...
    predicted_class = class_names[predicted_idx]
    confidence_score = all_probs[predicted_idx]
...
def test_onnx():
    """ONNX 모델 테스트"""
    # 설정
    onnx_model_path = "Bestefficientnet.onnx"
    image_path = "Tucson_preprocessed_cropped.png"

    try:
        # 예측
        predicted_class, confidence, all_probs = predict_onnx(
            image_path, onnx_model_path
        )
        ...
        for idx in top5_indices:
            print(f"{class_names[idx]}: {all_probs[idx]*100:.2f}%")
        print(f"{'='*50}\n")
...
def main():
    # 설정
    weight_path = config["WEIGHT"]  # "Efficientnet_bestmodel.pth"
    image_path = "testRay_cropped.png"

    try:
        # 모델 로드
        print("모델 로딩 중...")
        model, device = load_model(weight_path, len(class_names))
        ...
        print("상위 5개 예측:")
        print(f"{'-'*50}")
        top5_indices = np.argsort(all_probs)[::-1][: min(5, len(class_names))]
        for idx in top5_indices:
            print(f"{class_names[idx]}: {all_probs[idx]*100:.2f}%")
        print(f"{'='*50}\n")
...
#     Convert_ONNX()  # ONNX 변환
#     main()  # PyTorch 모델 테스트
#     test_onnx()  # ONNX 모델 테스트
